...
$(document).ready(() => {
    ...
    const loadPublicaciones = () => {
        $.ajax({
            url: ruta_devolver_publicaciones_perfil,
            type: 'POST',
            data: {
                "pagina": pagina,
                "userId": userId
            ...
            success: function (data) {
                console.log(data)
                data.forEach(d => {
                    $("#principal").append($(`<div id="publicacion-${d.id}" class="publicacion feed container" x-data="{comentarios: false}">
					<div class="container">
						<div class="card">
							<div class="container">
								<div
									...
									<div class="card">
										<div class="card-content">
											<div class="media">
												<div class="media-left">
													<figure class="image is-64x64 mr-0">
														<img class="is-rounded" src="${ruta_fotos + d.perfil}" alt="Imágen de perfil">
													</figure>
												</div>
											...
											<div class="content">
												<p>${d.texto}</p>
												${d.imagen ? `<img src="${ruta_fotos + d.imagen}" alt="Imagen publicacion">` : ''}
                                                ${d.video ? `<video controls><source src='${ruta_videos + d.video}'></video>` : ''}
											</div>
                                            ...
                                            <div class='comments comments-${d.id}' x-show="comentarios">
                                                ...
                                                <figure class="media-left">
                                                  <p class="image is-48x48">
                                                    <img class='is-rounded' src="${ruta_fotos + e.foto}">
                                                  </p>
                                                </figure>
                                                <div class="media-content">
                                                  <div class="content">
                                            ...
                                            <div id='personas-${d.id}' class='personas'>
                                                <h3>Likes:</h3>
                                                ${d.personas.map((x) => `<div class='persona-${x.id}'><img class='image is-48x48 is-rounded' src='${ruta_fotos + x.foto}'><span>${x.nombre}</span></div>`).join("")}
                                            </div>
										</div>
									</div>
                ...
                $(".crear-comment").unbind().click(function () {
                    ...
                    if ($(`#textarea-${id}`).val().length == 0) {
                        ...
                        $.ajax({
                            url: ruta_crear_comentario_perfil,
                            ...
                            success: function (data) {
                                ...
                                <figure class="media-left">
                                  <p class="image is-48x48">
                                    <img class='is-rounded' src="${ ruta_fotos + data.comentario.foto }">
                                  </p>
                                </figure>
                                <div class="media-content">
                                  <div class="content">
