$(document).ready(() => {
  ...
  $("#publicar-mensaje").click(() => {
    ...
    if (
      $("textarea").eq(0).val().replace(/\s/g, "").length == 0 &&
      document.getElementById("fileInput").files.length == 0
    ) {
      $("#error").remove();
      $(".control").before(
        $(
          `<p id='error' style='color: red; font-weight: bold' class='mt-4'>Es obligatorio subir un fichero/escribir un mensaje!</p>`
        )
      );
      function esconder() {
        $("#error").remove();
      ...
      success: function (data) {
        ...
        data.forEach((d) => {
          $("#principal").append(
            $(`<section id="publicacion-${
              d.id
            }" class="publicacion section feed" x-data="{comentarios: false}">
					<div class="container">
						<div class="card">
							<div class="container">
								<div
              ...
              d.autor
											...
											<div class="content">
                                                ...
                                                ${
                                                  d.video
                                                    ? `<video controls><source src='${d.video}'></video>`
                                                    : ""
                                                }
											</div>
											<div class="buttons" x-data="{borrar: false}">
												<button data-twt='${d.id}' class='btn-like button ${
              d.le_gusta ? "is-danger" : "is-light"
            }'>
													<span class="icon">
												...
												<button data-twt='${
                          d.id
                        }' @click="comentarios = !comentarios" class="btn-comment button" :class="comentarios ? 'is-light' : 'is-info'">
													<span class="icon">
                                                ...
                                                ${
                                                  correo == d.correo || es_admin
                                                    ? `<button @click="borrar = !borrar" x-show="!borrar"  class="button btn-borrar-post">
                                                <span class="icon">
                                            ...
                                            <div><button data-cod="${d.id}" @click="borrar = borrar" class="button btn-confirmar-borrar ml-3" x-show="borrar">
                                            ...
                                            <div class='comments comments-${
                                              d.id
                                                ...
                                                <div>${d.comentarios
                                                  .map(
                                                    (e) => `
                                                <article class="media comentario" id="comentario-${
                                                  e.id
                                                }">
                                                ...
                                                <div class="media-content">
                                                  <div class="content">
                                                    <p x-data="{borrar_comment: false}">
                                                      ...
                                                      <br>
                                                      <small><span id='likes-com-${
                                                        e.id
                                                      }'>${
                                                      e.likes
                                                    }</span> · <ion-icon data-com='${
                                                      e.id
                                                    }' class='${
                                                      e.le_gusta
                                                        ? "has-text-danger"
                                                        : "has-text-gray"
                                                    } btn-like-com ' name="heart"></ion-icon> · ${e.fecha.date.substring(
                                                      0,
                                                      16
                                                    )} ${
                                                      e.correo == correo ||
                                                      es_admin
                                                        ? ` · <ion-icon @click="borrar_comment = !borrar_comment" class="trash-icon" name="trash-outline" style='color: red;' x-show="!borrar_comment"></ion-icon>
                                            ...
                                            <div id='personas-${
                                              d.id
                                                ...
                                                ${d.personas
                                                  .map(
                                                    (x) =>
                                                      `<div class='persona-${x.id}'><img class='image is-48x48 is-rounded' src='${x.foto}'><span>${x.nombre}</span></div>`
                                                  )
                                                  .join("")}
                                            </div>
										</div>
          ...
          .click(function () {
            ...
            $.ajax({
              ...
              success: function (data) {
                ...
                $(`#nr-comentarios-${id_post}`).text(
                  Number($(`#nr-comentarios-${id_post}`).text()) - 1
                );
              },
              error: function (err) {
                console.log(err);
        ...
        $(".btn-like").hover(
          function (e) {
            ...
            let id = $(this).data("twt");
            if ($(`#personas-${id}`).children("div").length > 0) {
              $(`#personas-${id}`).show();
            }
          },
          function () {
        ...
        $(".crear-comment")
          ...
          .click(function () {
            ...
            if ($(`#textarea-${id}`).val().replace(/\s/g, "").length == 0) {
              $(`#textarea-${id}`).after(
                $(
                  `<p id='error' style='color: red; font-weight: bold' class='mt-4'>El campo de texto es obligatorio!</p>`
                )
              );
              ...
              $.ajax({
                ...
                success: function (data) {
                  console.log(data);
                  $(`#textarea-${id}`).val("");
                  $(`#nr-comentarios-${id}`).text(
                    Number($(`#nr-comentarios-${id}`).text()) + 1
                  );
                  $(`.comments-${id} > div`)
                    .prepend(`<article class="media comentario" id="comentario-${
                    data.comentario.id
                  }">
                                <figure class="media-left">
                                  <p class="image is-48x48">
                                    <img class='is-rounded' src="${
                                      data.comentario.foto
                                    }">
                                  </p>
                                </figure>
                                <div class="media-content">
                                  <div class="content">
                                    <p x-data="{borrar_comment: false}">
                                      <strong><a href="${ruta_perfil}/${
                    data.comentario.id_autor
                  }">${data.comentario.autor}</a></strong>
                                      <br>
                    ...
                    data.comentario.likes
                  }</span> · <ion-icon data-com='${
                    data.comentario.id
                  }' class='${
                    data.comentario.le_gusta
                      ? "has-text-danger"
                      : "has-text-gray"
                  } btn-like-com ' name="heart"></ion-icon> · ${data.comentario.fecha.date.substring(
                    0,
                    16
                  )} · <ion-icon @click="borrar_comment = !borrar_comment" class="trash-icon" name="trash-outline" style='color: red;' x-show="!borrar_comment"></ion-icon>
                                      <span x-show="borrar_comment">¿Estás seguro?</span>
                      
                                        <ion-icon name="checkmark-outline" data-com='${
                                          data.comentario.id
                                        }' data-post='${id}' class="trash-icon btn-confirmar-borrar-comment" style="color: green;" x-show="borrar_comment"></ion-icon>
                                        <ion-icon @click="borrar_comment = !borrar_comment" class="trash-icon" name="close-outline" x-show="borrar_comment"></ion-icon>
                                   </small>
                                    </p>
                                  </div>
